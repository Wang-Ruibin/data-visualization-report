...
add_para(
    ...
    "这个图我一开始考虑过分开画两张——一张柱状图看规模，一张折线图看普及率。"
    "但分两张的话对照着看就不方便，所以最后用了柱线组合图。"
    "柱状图展示网民的绝对规模，每一根柱子的高度代表当年的总人数，"
    "从柱子的高低起伏能直观地看出增长的速度变化。"
    "折线图叠加在上面展示普及率的变化趋势，普及率是相对值，"
    "用折线可以清楚地反映增长速度什么时候快、什么时候开始放缓。"
    "两个指标量纲不同（一个是亿、一个是百分比），"
    "所以普及率用了右侧的纵轴，左右各一套尺度，互不干扰。"
)

add_para(
    "前十年涨得特别快，每年基本多四五千万人，那会儿正好3G变4G、"
    "智能手机也越来越便宜。2020年后增速明显慢了，"
    "一年就多两三千万——该上的都上了，剩下的增量本来就不多。"
)
...
add_para(
    ...
    "手机网民的规模和占比是两个相关但量纲不同的指标，"
    "规模用亿来衡量，占比用百分比。"
    "双轴折线图正好处理这种情况——左侧纵轴标规模，右侧标占比，"
    "两条折线各自使用自己的尺度，互不影响。"
    "之所以选折线而不是柱状图，是因为数据是连续年份的变化趋势，"
    "折线能更好地反映上升的速率和拐点，"
    "比如2015年前后曲线斜率明显变陡，这个信息柱状图不容易看出来。"
)
...
add_para(
    ...
    "展示年龄构成的变化有好几种图可以选。"
    "饼图能看单一年份的构成，但五年的饼图排在一起很难比出各年龄段的变化。"
    "折线图能看变化趋势，但看不到每年的总体构成是什么样的。"
    "堆叠柱状图把这两种需求合在一起——每年是一根柱子，"
    "柱子里不同颜色代表不同年龄段，色块的高度就是该年龄段的占比，"
    "既能看每年的分布又能看跨年的结构变化。"
)
...
add_para(
    ...
    "学历是分类变量，类别之间没有高低顺序（不能说初中到高中是'递增'），"
    "用饼图或者环形图看比例最合适。"
    "这里选了环形图而不是传统饼图，"
    "主要是因为环形图中间没有饼心那一块，视觉上更清爽，"
    "而且五个类别不算多，环形图的各扇形角度很好辨认。"
    "如果类别超过七八个，环形图就会显得乱，"
    "但五个刚好合适。"
)
...
add_para(
    ...
    "这个图一开始我做的纵向柱状图，"
    "但应用名称有长有短，纵向的x轴标签叠在一起根本看不清。"
    "换成横向柱状图之后，应用名称写在每一条的旁边，"
    "多长的名字都能完整显示，而且按数值从大到小排列，"
    "第一名在最上面，一眼就能读出排名顺序。"
    "横向柱状图在展示排名类数据时确实比纵向好用，"
    "尤其是分类标签比较长的时候。"
)
...
add_para(
    ...
    "城乡对比用分组柱状图是因为它能同时做到两件事："
    "同一年城镇和农村两根柱子并排，差距有多大直接比出来；"
    "不同年份之间的变化也能通过柱子的走势看到。"
    "如果只用两条折线虽然也能看趋势，"
    "但具体到某一年城乡的具体差距就不如柱子直观。"
    "如果分开画两张图就更不好了，失去对比的意义。"
...
add_para(
    ...
    "这个数据用分组柱状图展示两组数据各自的趋势和对比关系。"
    "把IPv4和IPv6的柱子并排放，"
    "一条平线一条陡升的对比效果非常强烈。"
    "IPv6的增速在2017年之后明显加快，"
    "那一年国家专门发文部署IPv6规模部署，政策驱动效果可见一斑。"
    "分组柱状图在这里比堆叠柱状图更合适，"
    "因为IPv4和IPv6不是构成整体的部分，而是此消彼长的两个独立资源，"
    "并排放置比堆叠更能突出各自的独立变化。"
)
